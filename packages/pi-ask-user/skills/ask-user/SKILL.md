...
Use the `questions` wizard shape when one tool call should collect up to four closely related answers:
- `questions`: 1-4 items, each with `question`, optional `header`, optional `context`, optional `options`, optional `allowMultiple`, optional `allowFreeform`, and optional `allowComment`
- Keep every item focused and related to the same decision boundary.
- Put prerequisite or gating questions first.
- Later wizard questions may be skipped. Treat `null` responses as intentionally skipped or no longer applicable, not as a failed tool call.
- Avoid batching unrelated decisions just because the wizard exists.

### 5) Commit the decision
After response:
- restate the decision in plain language
- call out skipped wizard questions only when they affect implementation assumptions
...
- multiple unrelated decisions in one question or wizard
...
### Short wizard for conditional follow-ups

Use `questions[]` when follow-up prompts are tightly related and an earlier answer may make later prompts irrelevant. Skipped wizard questions are returned as `null`.

```json
{
  "context": "We can either stop after a safe default or collect rollout details now. If the user chooses the safe default, later rollout questions can be skipped.",
  "questions": [
    {
      "header": "Path",
      "question": "Which rollout path should we use?",
      "options": [
        { "title": "Safe default", "description": "Use the current conservative rollout and skip extra setup" },
        { "title": "Custom rollout", "description": "Collect rollout details now" }
      ]
    },
    {
      "header": "Environment",
    ...
    {
      "header": "Monitoring",
      "question": "If using a custom rollout, which monitoring threshold should gate promotion?",
...
- Packing unrelated decisions into `questions[]`
