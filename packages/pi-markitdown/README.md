...
A `read_raw` tool is also registered for accessing the original file content without conversion.
...
Use `read_raw` to bypass the conversion and get the original file content.
...
2. **`tool_result` hook on `read`**: Appends a footer to converted results noting the original file path and the `read_raw` escape hatch.
3. **`read_raw` tool**: Registered as a separate tool that reads files without any conversion.
4. **System prompt injection**: Adds a note to the system prompt explaining the enhanced `read` behavior and the `read_raw` escape hatch.
5. **Auto-install**: On session start, checks if `markitdown` is available. If not, creates a Python venv and installs `markitdown` with document format extras via pip.
6. **Cache invalidation**: Cache keys include the file's mtime, so re-reading a modified file triggers a fresh conversion.
